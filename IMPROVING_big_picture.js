...
// Ways to init tables each as a chrono figured out number
function OrganizeTables( n )
{
  if( n == 1 )  { OrganizeTables_Orig() }
  if( n == 2 )  { OrganizeTables_Special_8() }
}


// Do things for hooking tables to its wrap around package
function OrganizeTables_Special_8()
{
  ...
  // alert('OrganizeTables_Special_8');
  ...
  $(document).ready( function ( ) 
  {
    ...
    systemShape.forEach( function( daFacts )
    {
      ...
      table.MakeCellsEditable({ "onUpdate": editsCb, "paging": false });
    });

    // Happy talk
    // alert( "OrganizeTables_Special_8. Loops: " + aCount.toString() );
  });
}      
...
// Shape of transactions with server
...
// Change top line in current table 21mar21
